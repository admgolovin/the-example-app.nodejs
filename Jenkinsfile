        ...
        stage ('Deploy art'){
          steps{
            container('helm-cli'){
              script {
                sh "ls -a"
                currentSlot = sh(script: "helm get values --all js-app / | grep 'productionSlot:' | cut -d ' ' -f2 | tr -d '[:space:]'", returnStdout: true).trim()
                if (currentSlot == "blue") {
                    newSlot="green"
                    ...
                    sh "helm install -n js-app js_app/ --set image.deploy_blue=${revision},blue.enabled=true"
                    return
                  }
                        
                sh "helm upgrade js-app js_app/  --set ${tagVar}=${revision},${newSlot}.enabled=true --reuse-values"
                ...
                if (userInput == "y") {
                    sh "helm upgrade js-app js_app/  --set productionSlot=${newSlot} --reuse-values"
                }
                userInput = input(message: 'Delete old deployment? y\\n', parameters: [[$class: 'TextParameterDefinition', defaultValue: 'uat', description: 'Environment', name: 'env']])
                
                if (userInput == "y") {
                    sh "helm upgrade js-app js_app/  --set ${currentSlot}.enabled=false --reuse-values"
                  }
              }
            }
          }
